    ...
    for plugin in plugins.iter() {
        if let PluginType::Headless = plugin.run {
            let (instance, plugin_env) =
                start_plugin(plugin_id, plugin, 0, &bus, &store, &data_dir);
    ...
    loop {
        ...
        match event {
            PluginInstruction::Load(pid_tx, run, tab_index) => {
                ...
                let (instance, plugin_env) =
                    start_plugin(plugin_id, &plugin, tab_index, &bus, &store, &data_dir);
...
fn start_plugin(
    ...
    let output = Pipe::new();
    let input = Pipe::new();
    let stderr = LoggingPipe::new(&plugin.tag.to_string(), plugin_id);
    ...
    // Write the JSON options object to the plugin to be read on startup
